...
COPY . .
WORKDIR C:/src/src
RUN dotnet publish "FrameworkTests/AotConsole/AotConsole.csproj" -c %BUILD_CONFIGURATION% -r win-x64 -o C:/app/publish --no-restore -p:TargetFramework=net11.0 -p:PublishAot=false -p:SelfContained=true
